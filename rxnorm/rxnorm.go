...
// the HTTP client, request shaping, and the typed data models for rxnorm.
//
...
// DefaultUserAgent identifies the client to rxnorm. A real, honest
// User-Agent is both polite and the thing most likely to keep you unblocked.
const DefaultUserAgent = "rxnorm/dev (+https://github.com/tamnd/rxnorm-cli)"

// Host is the site this client talks to, and the host the URI driver in
// domain.go claims. The scaffold points it at rxnorm.com; change it once you
// know the real endpoints you want to read.
const Host = "rxnorm.com"

// BaseURL is the root every request is built from.
const BaseURL = "https://" + Host

// Client talks to rxnorm over HTTP.
type Client struct {
	HTTP      *http.Client
	UserAgent string
...
// path that names it on rxnorm.com. It is a stand-in for the typed records you
// will model from the real rxnorm endpoints.
//
// The kit struct tags make it addressable as a resource URI (see domain.go): ID
// is the URI id, and Body is the long text `rxnorm cat` and the Markdown
...
// a row, though it still rides in `-o json` and `rxnorm cat`. Swap `-` for
...
// rxnorm.com page URI a host can follow. Like the rest of the scaffold it is a
