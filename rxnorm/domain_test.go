...
// HTTP behaviour is covered in rxnorm_test.go.
...
func TestHostWiring(t *testing.T) {
	...
	if want := "rxnorm://page/wiki/Go"; u.String() != want {
	...
	if !h.Searchable("rxnorm") {
		t.Error("Searchable = false, want true (the domain registers a search op)")
	}

	got, err := h.ResolveOn("rxnorm", "about")
	if err != nil || got.String() != "rxnorm://page/about" {
		t.Errorf("ResolveOn = (%q, %v), want rxnorm://page/about", got.String(), err)
	}
}
