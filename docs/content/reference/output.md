...
describes what users get. Pick a format with `-o`, or let rxnorm choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
rxnorm <command> -o table     # a rounded, color-aware grid for reading
rxnorm <command> -o markdown  # a GitHub pipe table to paste into docs (alias: md)
rxnorm <command> -o list      # one record per section, easy on the eyes
rxnorm <command> -o jsonl     # one JSON object per line, for piping
rxnorm <command> -o json      # a single JSON array
rxnorm <command> -o csv       # spreadsheet friendly
rxnorm <command> -o tsv       # tab-separated
rxnorm <command> -o url       # just the URL column
rxnorm <command> -o raw       # the underlying bytes, unformatted
```
...
```bash
rxnorm <command> --fields id,title,url
```
...
```bash
rxnorm <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
rxnorm <command>            # a table, because this is a terminal
rxnorm <command> | wc -l    # JSONL, because this is a pipe
```
